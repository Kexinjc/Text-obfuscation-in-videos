...
def main(input, output, recognizer, exclude_recognizer, filter, unfilter, verbose):
    ...
    if input.endswith(('.jpg', '.jpeg', '.png')):
        ...
        # run video pipeline
        video_pipeline(image_preprocessor, tesseract_api, analyser, anonymiser, input, output, verbose)
...
def video_pipeline(image_preprocessor: OCRPreprocessor, tesseract_api: PyTesseractAPI, analyser: OCRAnalyser, anonymiser: Anonymiser, input: str, output: str, verbose: bool):
    # read video
    ...
    # create video writer
    out = cv2.VideoWriter(output, cv2.VideoWriter_fourcc(*'mp4v'), frame_rate, (frame_width, frame_height))
